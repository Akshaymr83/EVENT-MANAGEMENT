...
const Booking = ({ userId }) => {
    ...
    const handleFinalSubmit = async () => {
      ...
      if (!userId) {
          console.error('User ID is undefined. Cannot proceed with booking.');
          alert('User ID is required to submit the booking.');
          return; // Exit the function if userId is not set
      }
  
      const bookingData = {
          ...formData,
          bookingDate: formData.bookingDate.toISOString(), // Ensure the date is in ISO format
          user: userId, // Pass the userId correctly
      };
  
      // Log the booking data to verify its structure and values
      console.log('Booking data:', bookingData);
      ...
      try {
          ...
          alert("Booking Confirmed!");
          navigate("/thank-you"); // Redirect after successful booking
      } catch (error) {
          if (error.response) {
              console.error("Error submitting booking:", error.response.data);
              alert(`Failed to confirm booking: ${error.response.data.message}`);
